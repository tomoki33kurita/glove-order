export const stitch = (ctx: CanvasRenderingContext2D, color: string, webType: string): void => {
  ctx.lineWidth = 1.5
  ...
  if (webType === 'h_web') {
    ...
    // Hウェブ縦＿左＿外側
    ctx.moveTo(243, 237) // 上
    ctx.quadraticCurveTo(253, 253, 253, 253) // 下
    // Hウェブ縦＿左＿内側
    ctx.moveTo(245, 233) // 上
    ctx.quadraticCurveTo(257, 250, 253, 250) // 下
    // Hウェブ縦＿右＿内側
    ctx.moveTo(277, 178) // 上
    ctx.quadraticCurveTo(282, 194, 282, 194) // 下
    // Hウェブ縦＿右＿外側
    ctx.moveTo(280, 175) // 上
    ctx.quadraticCurveTo(286, 195, 286, 195) // 下
  }
